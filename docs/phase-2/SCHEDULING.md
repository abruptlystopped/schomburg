...
Manual **Update Record** and scheduled Daily Reconciliation are distinct
operations. Both reuse collection, storage, and record generation, but manual
status never satisfies or changes scheduled reconciliation status. Scheduled
success records the eligible local date even for a no-op run.
